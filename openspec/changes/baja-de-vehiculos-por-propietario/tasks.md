...
- [ ] 6.3 Desplegar junto con `seguimiento-automatico-de-difusiones`: migraciones 524 y la de este cambio, `up -d --build` y recreación del contenedor del cron
- [ ] 6.4 En producción, después de importar los contactos: generar el SQL de propietarios con el script, revisarlo con el usuario, aplicarlo solo con autorización explícita, y revisar el informe de placas sin vehículo
- [ ] 6.5 Agregar el paso "Ocultar el vehículo del contacto" a la automatización del botón `NO`, después de la etiqueta `propietario-vendido`
