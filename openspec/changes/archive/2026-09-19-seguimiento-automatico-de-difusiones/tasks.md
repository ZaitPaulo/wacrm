...
- [x] 6.3 Desplegar en el VPS: respaldo, `git pull`, migración aplicada, `up -d --build`, y **recrear el contenedor del cron**; verificar en su log la línea `ok /api/broadcasts/cron` (2026-09-14: respaldo `20260914-182246`, 524 y 525 aplicadas, app `9847307` healthy; un tick manual con `tick.sh` desde el contenedor devolvió `ok /api/broadcasts/cron`. Los ticks automáticos no corrían por el problema de la 6.6)
- [ ] 6.4 En producción, antes de la campaña real: crear y hacer aprobar en Meta la plantilla del recordatorio con los mismos botones SI / NO, y probar con una difusión a un contacto propio (2026-09-15: plantilla `recordatorio_consulta_vehiculo_disponible` aprobada, es_CO, botones `SI`/`NO` verificados en la base. Falta la difusión de prueba a un contacto propio)
- [x] 6.6 Arreglar el cron del VPS, que no ejecutaba ninguna tarea (hallado el 2026-09-15 al verificar el despliegue): busybox crond solo carga crontabs cuyo dueño es root, y el crontab estaba montado desde el host con uid 1001. Montar `deploy/cron` como directorio, copiar `crontab` y `tick.sh` al arrancar, y verificar en `docker logs crm-cron` un `ok /api/automations/cron` por minuto y un `ok /api/broadcasts/cron` cada 5
