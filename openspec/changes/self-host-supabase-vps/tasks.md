...
- [x] 4.4 Documentar los registros DNS necesarios y las reglas de firewall (solo 22/80/443 abiertos)
- [x] 4.5 Construir la imagen del app en el servidor con los `NEXT_PUBLIC_*` definitivos y levantarla
- [x] 4.6 Verificar TLS válido en los tres hosts desde una red externa
...
- [x] 9.5 Guardar fuera del servidor una copia de `deploy/.env` y `.env.local`, y dejar registrado dónde quedó — **`.env.local` no existe en el servidor**: el único archivo de secretos es `deploy/.env` (123 líneas), y `deploy/supabase/.env` es un enlace simbólico a él. Copiado el 2026-08-24 a la máquina de trabajo, fuera del repo, con el sha256 verificado contra el original. La ruta exacta va en la memoria del proyecto, no aquí
