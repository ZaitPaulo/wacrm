...
- **Composición de la publicación** a partir de los datos que ya tiene el vehículo: fotos, precio, año, kilometraje y ficha técnica.
...
**Código**
- `src/lib/instagram/` — cliente de Graph API para publicación, siguiendo el estilo de `src/lib/whatsapp/meta-api.ts`.
...
- **Tope de publicaciones de Meta.** Hay un máximo por día; superarlo devuelve error. El sistema debe conocerlo y no gastarlo a ciegas.
- **Permisos y revisión de la app.** Publicar exige permisos que Meta revisa, y la cuenta debe ser Instagram Business o Creator vinculada a una página de Facebook. Una cuenta personal no sirve. Es calendario ajeno y bloquea las pruebas reales.
