...
import { InAppNotificationAlerts } from "@/components/notifications/in-app-notification-alerts";
...
function DashboardShellInner({ children }: { children: React.ReactNode }) {
  ...
  // `h-dvh`, no `h-screen`. En Chrome Android `100vh` es el viewport
  // *grande* —el que existe cuando la barra de direcciones está oculta—,
  // así que mientras esa barra se ve el documento mide más que la
  // pantalla: el body se desplazaba ~60px y el header de acá abajo
  // quedaba fuera de vista, sin forma de abrir el menú lateral. `dvh`
  // sigue al viewport dinámico y el contenedor siempre mide lo que se ve.
  return (
    <div className="flex h-dvh overflow-hidden bg-background">
      {/* Reports this tab's online/away presence once we know a user is
          signed in. Headless — renders nothing. */}
      <PresenceHeartbeat />
      {/* Aviso emergente + sonido con el CRM abierto. Headless. Suspense
          porque lee la conversación abierta de `?c=` (useSearchParams). */}
      <Suspense fallback={null}>
        <InAppNotificationAlerts />
      </Suspense>
