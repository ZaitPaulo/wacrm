...
async function processMessage(
  ...
  const { data: insertedRows, error: msgError } = await supabaseAdmin()
    ...
    // Upstream selecciona sólo el id; este fork necesita ambos.
  ...
  // `insertedRows[0]` siempre existe aquí: el guard de reenvío de más
  // arriba ya devolvió cuando el arreglo venía vacío.
...
async function findOrCreateConversation(
  ...
  configOwnerUserId: string,
  contactId: string,
  // Desde la 513 un contacto puede tener un hilo por canal. TODA
  // consulta de conversación por contacto tiene que acotarse por canal:
  // sin eso, apenas exista el primer hilo de Instagram de alguien que ya
  // tenía WhatsApp, esta búsqueda devolvería el hilo equivocado — y las
  // que usan `.single()` volverían a fallar en cada mensaje, que es
  // exactamente el bug #363 que motivó la 036.
