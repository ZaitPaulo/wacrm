    ...
    from(table: string) {
      switch (table) {
        ...
        case 'conversations':
          ...
          //
          // El tercer .eq() es el filtro por canal (migración 513). Sin
          // él la búsqueda podría devolver el hilo de otro canal del
          // mismo contacto.
          return {
            select: () => ({
              eq: () => ({
...
// ============================================================
// Enrutado por tipo de evento (multicanal, migración 513).
//
// Los tres canales de Meta llegan a ESTA misma URL y se distinguen por
// el campo `object` del cuerpo. Lo que se fija acá es que reconocer un
// canal que todavía no tiene manejador no rompa nada, y que un lote con
// varios eventos no se pierda entero por culpa de uno.
// ============================================================
...
async function runRaw(body: unknown) {
  const res = await POST(rawRequest(body));
  for (const cb of h.state.afterCallbacks) await cb();
  return res;
}

/** Un cambio de WhatsApp válido, para armar lotes. */
function whatsappChange(messageId: string) {
  return {
    field: 'messages',
    value: {
      metadata: { phone_number_id: 'pn-1' },
      contacts: [{ wa_id: '15551230000', profile: { name: 'Ada' } }],
      messages: [{ ...TEXT_MESSAGE, id: messageId }],
    },
  };
}

describe('webhook: enrutado por canal', () => {
  it('procesa WhatsApp cuando el object lo declara', async () => {
    await runRaw({
      object: 'whatsapp_business_account',
      entry: [{ changes: [whatsappChange('wamid.A')] }],
    });

    expect(h.state.upsertCalls.length).toBeGreaterThan(0);
  });

  it('sigue procesando un cuerpo sin object, como los de antes', async () => {
    // Las instalaciones existentes ya mandaban eventos que este código
    // procesaba sin mirar `object`. Ninguna puede dejar de funcionar
    // por un campo que antes se ignoraba.
    await runRaw({ entry: [{ changes: [whatsappChange('wamid.B')] }] });

    expect(h.state.upsertCalls.length).toBeGreaterThan(0);
  });

  it('descarta un canal sin manejador sin procesar nada ni lanzar', async () => {
    const res = await runRaw({
      object: 'instagram',
      entry: [{ messaging: [{ sender: { id: 'ig-1' } }] }],
    });

    // Responde con éxito: un error haría que Meta reintente en bucle.
    expect((res as unknown as { init: { status: number } }).init.status).toBe(
      200
    );
    expect(h.state.upsertCalls).toHaveLength(0);
  });

  it('descarta un object desconocido sin lanzar', async () => {
    await expect(
      runRaw({ object: 'algo_que_no_existe', entry: [] })
    ).resolves.toBeDefined();
    expect(h.state.upsertCalls).toHaveLength(0);
  });
});

describe('webhook: un evento roto no se lleva el lote', () => {
  it('procesa los demás cambios cuando uno falla', async () => {
    // El primer cambio viene sin `value`, así que leerlo lanza. El
    // segundo es válido y tiene que llegar a persistirse igual.
    await runRaw({
      object: 'whatsapp_business_account',
      entry: [
        {
          changes: [{ field: 'messages' }, whatsappChange('wamid.SOBREVIVE')],
        },
      ],
    });
  ...
  it('procesa el segundo entry cuando el primero falla', async () => {
    await runRaw({
      object: 'whatsapp_business_account',
      entry: [
        { changes: [{ field: 'messages' }] },
        { changes: [whatsappChange('wamid.SEGUNDO')] },
      ],
    });
  ...
  it('tolera un entry sin changes', async () => {
    await expect(
