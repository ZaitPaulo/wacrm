...
export function ConversationList({
  ...
  // Canales presentes en la bandeja. El filtro SOLO aparece cuando hay
  // más de uno: en una cuenta que solo usa WhatsApp sería un control
  // que nunca cambia nada y estorba a la vista.
  const channelsInUse = useMemo(() => {
    const seen = new Set<MessageChannel>();
    for (const c of conversations) seen.add(c.channel ?? 'whatsapp');
    return [...seen].sort();
  }, [conversations]);

  const toggleChannel = useCallback((channel: MessageChannel) => {
