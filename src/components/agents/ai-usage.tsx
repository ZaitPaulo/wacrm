...
export function AiUsageCard() {
  ...
  const fetchUsage = useCallback(async (windowDays: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(json?.error ?? t('usage.loadFailed'));
        setData(null);
        return;
      }
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <CardTitle className="flex items-center gap-2 text-base">
              <BarChart3 className="h-4 w-4 text-primary" /> {t('usage.title')}
            </CardTitle>
            <CardDescription>
              {t('usage.description')}
                        </CardDescription>
      ...
      <CardContent className="space-y-5">
        {loading || !data ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center text-sm text-muted-foreground">
            ...
            <p>{t('usage.emptyTitle', { count: data.window_days })}</p>
            <p className="text-xs">
              {t('usage.emptyHint')}
                        </p>
          </div>
          ...
          <>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <Stat label={t('usage.totalTokens')} value={formatCompactNumber(data.totals.total_tokens)} />
              <Stat label={t('usage.llmCalls')} value={String(data.totals.calls)} />
              <Stat
                label={t('usage.autoReply')}
                value={formatCompactNumber(data.by_mode.auto_reply.tokens)}
                icon={Bot}
              />
              <Stat
                label={t('usage.drafts')}
            ...
            <div>
              <p className="mb-2 text-xs font-medium text-muted-foreground">
                {t('usage.tokensPerDay')}
              </p>
              <BarChart
                data={chartData}
                index="day"
                categories={[t('usage.tokens')]}
                colors={['violet']}
